...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $stmt->bind_param("i", $gatepass_id);
            $stmt->execute();
              // Get returnable checkbox values
            $item_returnable = isset($_POST['item_returnable']) ? $_POST['item_returnable'] : array();
            
            // Insert new items
            $stmt = $conn->prepare("
                INSERT INTO gatepass_items (gatepass_id, item_name, quantity, unit, is_returnable) 
                VALUES (?, ?, ?, ?, ?)
            ");
            
            for ($i = 0; $i < count($item_names); $i++) {
                $is_returnable = in_array((string)$i, $item_returnable) ? 1 : 0;
                $stmt->bind_param("isdsi", $gatepass_id, $item_names[$i], $item_quantities[$i], $item_units[$i], $is_returnable);
                $stmt->execute();
            }
                                ...
                                <tr class="item-row">
                                    ...
                                    <td>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="item_returnable[]" 
                                                   value="<?php echo $item_count - 1; ?>" 
                                                   <?php echo isset($item['is_returnable']) && $item['is_returnable'] == 1 ? 'checked' : ''; ?>>
    ...
    addItemButton.addEventListener('click', function() {
        ...
        newRow.className = 'item-row';
          newRow.innerHTML = `
            ...
            <td>
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="item_returnable[]" value="${itemCount}">
                    <label class="form-check-label">Returnable</label>
                </div>
            </td>
            <td>
