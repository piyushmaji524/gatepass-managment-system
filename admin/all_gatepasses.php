...
$query = "
    SELECT g.*, 
           ...
           (SELECT COUNT(*) FROM gatepass_items gi WHERE gi.gatepass_id = g.id AND gi.is_returnable = 1) as returnable_count,
           (SELECT COUNT(*) FROM gatepass_items gi WHERE gi.gatepass_id = g.id AND gi.is_returnable = 1 AND gi.returned = 0) as pending_returns_count
    FROM gatepasses g
...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-hover table-striped table-export">                <thead class="table-light">
                    <tr>
                        <th>Gatepass #</th>
                        <th>From → To</th>
                        <th>Material</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($gatepass['returnable_count'] > 0): ?>
                                <span class="badge bg-info"><?php echo $gatepass['returnable_count']; ?> Returnable</span>
                                <?php if ($gatepass['pending_returns_count'] > 0): ?>
                                    <br>
                                    <span class="badge bg-warning text-dark mt-1"><?php echo $gatepass['pending_returns_count']; ?> Pending</span>
                                <?php else: ?>
                                    <br>
                                    <span class="badge bg-success mt-1">All Returned</span>
                                <?php endif; ?>
                            <?php else: ?>
