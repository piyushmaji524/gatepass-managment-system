...
class GatepassPDF extends FPDF {    // Page header
    ...
    function Footer() {
        ...
        // Add version to help with debugging
        $this->SetY(-5);
        $this->SetFont('Arial', 'I', 6);
        $this->Cell(0, 10, 'v1.1', 0, 0, 'R');
    }
    ...
    function ItemsTable($items) {
        ...
        $this->Cell(70, 8, 'Item Name', 1, 0, 'C', true);
        $this->Cell(30, 8, 'Quantity', 1, 0, 'C', true);
        $this->Cell(30, 8, 'Unit', 1, 0, 'C', true);
        $this->Cell(30, 8, 'Returnable', 1, 0, 'C', true);
        ...
        while ($item = $items->fetch_assoc()) {
            ...
            $this->Cell(70, 8, $item['item_name'], 1, 0, 'L');
            $this->Cell(30, 8, $item['quantity'], 1, 0, 'R');
            $this->Cell(30, 8, $item['unit'], 1, 0, 'C');
            
            // Returnable status - Check using isset to prevent PHP notices
            $is_returnable = isset($item['is_returnable']) ? (int)$item['is_returnable'] : 0;
            if ($is_returnable == 1) {
                $this->SetTextColor(23, 162, 184); // Info blue color
                $this->Cell(30, 8, 'Yes', 1, 0, 'C');
                $this->SetTextColor(0, 0, 0); // Reset to black
                
                // Return status
                $is_returned = isset($item['returned']) ? (int)$item['returned'] : 0;
                if ($is_returned == 1) {
                    $this->SetTextColor(40, 167, 69); // Success green
                    $this->Cell(30, 8, 'Returned', 1, 1, 'C');
                } else {
                    $this->SetTextColor(255, 193, 7); // Warning yellow
                    $this->Cell(30, 8, 'Pending', 1, 1, 'C');
                }
                $this->SetTextColor(0, 0, 0); // Reset to black
            } else {
                $this->SetTextColor(108, 117, 125); // Secondary gray
                $this->Cell(30, 8, 'No', 1, 0, 'C');
                $this->Cell(30, 8, 'N/A', 1, 1, 'C');
                $this->SetTextColor(0, 0, 0); // Reset to black
            }
    ...
    function ReturnableItemsSummary($items) {
        // Reset pointer
        $items->data_seek(0);
        
        // Count returnable items
        $returnable_count = 0;
        $returned_count = 0;
        $pending_count = 0;
        
        while ($item = $items->fetch_assoc()) {
            // Use explicit type conversion to ensure consistent comparison
            $is_returnable = isset($item['is_returnable']) ? (int)$item['is_returnable'] : 0;
            if ($is_returnable == 1) {
                $returnable_count++;
        ...
        // If there are returnable items, show summary
        if ($returnable_count > 0) {
            $this->SetFont('Arial', 'B', 12);
            $this->Cell(0, 10, 'RETURNABLE ITEMS SUMMARY', 0, 1);
            
            $this->SetFont('Arial', '', 10);
            $this->SetFillColor(240, 240, 240);
            
            $this->Cell(190, 8, 'Total Returnable Items: ' . $returnable_count, 1, 1, 'L', true);
            
            if ($returned_count > 0) {
                $this->SetTextColor(40, 167, 69); // Success green
                $this->Cell(190, 8, 'Returned Items: ' . $returned_count, 1, 1, 'L', true);
                $this->SetTextColor(0, 0, 0); // Reset to black
            }
            
            if ($pending_count > 0) {
                $this->SetTextColor(255, 193, 7); // Warning yellow
                $this->Cell(190, 8, 'Pending Returns: ' . $pending_count, 1, 1, 'L', true);
                $this->SetTextColor(0, 0, 0); // Reset to black
            }
